...
Alors que Java 8 sortait il y a un peu plus de 11 ans en mars 2014, il apportait une fonctionnalité qui allait significativement impacter la façon dont on allait écrire notre code grâce à l'API Stream et aux expressions lambdas.
...
Bien que nombreuses, les opérations intermédiaires proposées par l'API ne répondent pas à tous les cas d'usages.
...
L'API Stream propose de nombreuses opérations terminales telles que `forEach()`, `findFirst()`, `count()`, ... répondant chacune à un besoin courant spécifique.
Il est également possible de définir n'importe quelle opération terminale en définissant sa propre implémentation de `Collector<? super T, A, R> collector` et en la fournissant comme paramètre de l'opération terminale `collect()`.
...
Comme son nom l'indique, si l'invocation de la méthode renvoie `true`, le _downstream_ n'accepte plus de nouveau élément.
...
L'interface `Integrator` fournit la fabrique `ofGreedy()` permettant d'obtenir une instance d'_integrator_ conçu pour consommer l'intégralité de son input (si tenté que le _downstream_ continue d'accepter des éléments). +
