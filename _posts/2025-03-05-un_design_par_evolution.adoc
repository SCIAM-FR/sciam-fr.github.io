...
En plus des classiques algorithmes génétiques, stratégies d'évolution, programmation génétique, elle propose des algorithmes évolutionnaires multi-objectifs (MOEA) comme NSGA-IIfootnote:nsga[*NSGA-II (Non-dominated Sorting Genetic Algorithm II)* est un algorithme génétique multi-objectifs largement utilisé en recherche opérationnelle et en informatique.
