...
Cette API permet de créer des prompts, constituées de messages, qui sont envoyées au modèle pour guider sa réponse et son comportement.
...
NOTE: Les modèles (LLM) sont formé pour détecter quand appeler une fonction et répondre avec un `JSON` conforme à la signature de la fonction définie.
