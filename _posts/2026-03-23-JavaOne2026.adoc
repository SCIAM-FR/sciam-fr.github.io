...
Ce qui a frappé les participants, c'est à quel point LangChain4j-CDI fait de l'IA un citoyen de première classe du modèle de programmation CDI. L'idée fondamentale est que l'IA ne devrait pas être un corps étranger dans votre application, elle doit s'exprimer avec les mêmes concepts que le reste de votre code.
...
Le `ChatMemoryProvider` est la clé de la mémoire conversationnelle : c'est lui qui fournit un `ChatMemory` par utilisateur, permettant au LLM de conserver le contexte d'une conversation. Et comme le provider est lui-même un bean CDI, il peut injecter un cache, un store persistant ou tout autre mécanisme de votre SI pour gérer le cycle de vie de cette mémoire.
...
Le LLM peut appeler `checkStock`, et derrière, c'est CDI qui injecte le `Repository`, gère la transaction, applique les intercepteurs. L'outil IA bénéficie de toute l'infrastructure enterprise sans aucun câblage spécifique.

Et c'est là que la convergence avec MicroProfile devient particulièrement élégante. Parce que votre AI Service est un bean CDI, vous pouvez lui appliquer directement les intercepteurs MicroProfile :
...
public interface ResilientAssistant {
    ...
    default String fallbackAnswer(String question) {
        return "Service temporairement indisponible, veuillez réessayer.";
    }
}
----

Pas de wrapper, pas de proxy custom : c'est le mécanisme standard d'intercepteurs CDI qui s'applique. La résilience, l'observabilité via MicroProfile Telemetry, la configuration via MicroProfile Config, tout fonctionne nativement parce que l'AI Service *est* un bean CDI.

C'est exactement le pari de LangChain4j-CDI : il n'y a pas de rupture paradigmatique entre le CDI que vous pratiquez et l'IA que vous voulez intégrer. L'IA dans le SI enterprise ne devrait pas nécessiter une révolution de stack, mais une évolution naturelle du modèle de programmation que vous maîtrisez déjà.
