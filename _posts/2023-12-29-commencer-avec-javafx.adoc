...
Le SceneGraph est une représentation hiérarchique des composants graphiques de notre interface. +
...
JavaFX est un framework _mono-threadé_, ce qui signifie que toutes les intéractions avec les composants graphiques doivent se faire depuis le `JavaFX Application Thread`. A l'instar de Swing, on dispose d'une API permettant de réaliser ces opérations au sein de ce thread, à savoir `Platform.runLater()`. +
...
Gluon fournit avec sa bibliothèque _Ignite_ une solution pour les frameworks `Spring`, `Guice` et `Dagger`. +

https://github.com/gluonhq/ignite[Gluon Ignite]
