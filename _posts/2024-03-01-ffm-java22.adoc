...
:page-excerpt: JNI est mort, vive FFM ! Java 22 propose (enfin) en standard son API FFM : "Foreign Function a Memory". Son but est de fournir un moyen sûr et performant de manipuler des données en dehors de la heap memory et d'intéragir avec du code natif.
:layout: post
...
Depuis le JDK 1.1, il est possible de manipuler des données "off-heap" et d'intéragir avec du code natif via JNI "Java Native Interface". L'utilisation du modificateur `native` permet l'invocation de fonctions natives, moyennement une succession d'étapes parfois périlleuses.
...
Ces APIs présentent un certain nombre d'inconvénients : `ByteBuffer` limite l'accès à une zone de 2Go et les méthodes permettant d'y accéder restent rudimentaires.
...
FFM a pour but de fournir un moyen sûr et performant de répondre à ces problématiques.
...
Il aura donc fallu pas moins de 4 années pour voir arriver ces fonctionnalités en standard dans le JDK 22 au travers de la https://openjdk.org/jeps/454[JEP 454].
...
On va pouvoir déclarer une méthode java qui sera appelée comme callback de trace. La signature de la méthode java doit correspondre son homologue natif.
...
On peut désormais créer l'_upcall_ grâce au `Linker`. Ce dernier va se charger de créer un pointeur de fonction.
...
try {
    int traceCode = (int) traceHandle.invokeExact( // <4>
        dbPtr,
        0x01, // SQLITE_TRACE_STMT <5>
        upcallStub,
        MemorySegment.NULL
    );
...
<4> Invocation de la fonction native
<5> Événement déclenchant une trace, ici la valeur SQLITE_TRACE_STMT
...
https://jdk.java.net/jextract/[JExtract] est un outil du projet OpenJDK dont le but est la génération automatique d'un _binding_ java depuis les fichiers headers natifs.
...
----
<1> Définition du callback java, pour le besoin de l'exemple, on ignore les paramètres (cf https://openjdk.org/jeps/456[JEP 456], fraîchement en standard depuis cette version 22 du JDK)
...
FFM y remédie avec l'interface `MemoryLayout` qui permet de définir une structuration de la donnée et d'y accéder de manière simplifiée.
