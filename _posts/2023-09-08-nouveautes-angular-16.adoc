...
----
L'opérateur `toSignal` permet de convertir l'Observale d'un service utilisant le module HttpClientModule(Get, Post, ..). La récupération de données dans le controller sera plus simple avec la déclaration de Signal qui reçoit la valeur de retour de toSignal.
...
== Rxjs-interpo
  ...
  Un nouveau module d'angular propose des opérations qui conviennent avec le le système de réactivité basé sur le signal d'Angular.
