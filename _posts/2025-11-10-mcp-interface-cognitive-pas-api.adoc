...
3. *Prompts* : templates d'instructions préconfigurés que les utilisateurs peuvent invoquer via l'application cliente.
...
*OpenAPI décrit la syntaxe* : une API REST avec OpenAPI spécifie exactement quels endpoints existent, quels verbes sont supportées, quels formats de données sont attendus (JSON, XML, ...) en entrée et en sortie.
...
Dans REST, une ressource est une entité identifiable par des URI (`/articles/123`).
