...
Les ancienne version d'angular on utilise *takeUntil* de la bibliothéque Rxjs, pour détruire un observable. Ici, on est obligé de déclarer *destroyed$* comme un subject et le compléter dans ngOnDestry.


== Conclusion 

Avec cette nouvelle version 16, il y a deux grandes phases d'amélioration, en premier lieu est l'hydratation pour réduire le temps de chargement et en dexiéme le Signal où angular a ajouté pas mal des bibliothéques qui tournent autour de Signal.

Et j'ai adoré le faite que le code sera plus simple et moins compliqué que les ancienne versions :D. 
