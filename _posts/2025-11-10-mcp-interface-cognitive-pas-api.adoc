...
Bien que les LLM soient intrinsèquement stateless, (ils ne conservent pas de mémoire entre les appels), l'application cliente (l'host MCP) maintient l'historique de la conversation et le réinjecte dans le contexte à chaque nouvelle requête.
...
Cette continuité contextuelle, orchestrée par le client MCP qui réinjecte l'historique à chaque appel, n'existe pas dans une API REST classique.
OpenAPI ne peut pas décrire comment un tool s'insère dans un flux conversationnel multi-appels où chaque appel dépend potentiellement des résultats précédents conservés en mémoire par l'application cliente.
