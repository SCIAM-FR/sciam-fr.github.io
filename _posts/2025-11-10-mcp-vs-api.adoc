...
public class BlogMcpServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 3. Définir le tool de recherche d'articles
        McpServerFeatures.SyncToolSpecification searchTool =
            new McpServerFeatures.SyncToolSpecification(
                new McpSchema.Tool(
                    "search_articles",
                    "Recherche des articles de blog par type ou mot-clé",
                    """
                    {
                      "type": "object",
                      "properties": {
                ...
                (exchange, arguments) -> {
                    ...
                    List<Article> results = searchInDatabase(query, type);

                    String jsonResults = new ObjectMapper()
                        .writeValueAsString(results);

                    return new McpSchema.CallToolResult(
                        List.of(new McpSchema.TextContent(jsonResults)),
                        false
                    );
                }
            );

        // 4. Définir le tool de création d'article
        McpServerFeatures.SyncToolSpecification createTool =
            new McpServerFeatures.SyncToolSpecification(
                ...
                (exchange, arguments) -> {
                    ...
                    Article created = createArticle(title, content, type);

                    return new McpSchema.CallToolResult(
                        List.of(new McpSchema.TextContent(
                            "Article créé avec succès : " + created.getId()
                        )),
                        false
                    );
                }
        ...
        // 5. Enregistrer les tools
        server.addTool(searchTool);
        server.addTool(createTool);

        // 6. Le serveur est prêt à recevoir des connexions
        System.err.println("Blog MCP Server démarré et en attente de connexions");
        // Le serveur écoute automatiquement sur stdin/stdout
    }

    private static List<Article> searchInDatabase(String query, String type) {
        // Implémentation de la recherche
        return ArticleRepository.search(query, type);
    }

    private static Article createArticle(String title, String content, String type) {
        // Implémentation de la création
        return ArticleRepository.create(title, content, type);
    }
}
----
