...
Nous utilisons le décorateur @Input dans le projet Angular pour passer des variables de composant parent au composant fils. Maintenant, nous avons la possibilité d'utiliser le signal avec input() pour assurer une communication plus réactive. 

En utilisant le signal input(), il est possible de dériver l'entrée sans implémenter la fonction ngOnchanges. Le traitement peut être effectué dans le constructor avec effect().
...
----
 ...
 <1> Déclarer le surname en tant que Signal input : c'est une variable qu'on récupere de  routing  et on va envoyer au composant fils "Ajouter un nouveau utilisateur" 
...
*Option 1 : en lancent la commande ng new my-app : Angular cli demande d’utiliser SSR/SSG / Prerendring 
