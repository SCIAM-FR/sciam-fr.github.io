...
Cela permet d’aller vite au démarrage, surtout dans des environnements complexes.  
En revanche, certaines incohérences ne sont détectées que plus tard :

* lors d’un appel réel
* en phase d’intégration
* ou sous charge

Autrement dit, *l’application fonctionne jusqu’à ce qu’elle ne fonctionne plus*.

Quarkus adopte une approche différente en déplaçant un maximum de vérifications au moment du build :
...
Ces validations s’appuient sur des mécanismes comme l’indexation des classes et la génération de bytecode.  
L’objectif est simple : *réduire au maximum le travail effectué au runtime*.

Le résultat est direct : *ce qui serait une erreur runtime en Spring devient une erreur de build en Quarkus*.
...
Cette logique s’applique aussi à des cas fréquents :
...
Spring tolère davantage et reporte certaines validations.  
Quarkus impose une cohérence dès le départ.

=== Impact sur le développement

Ce changement modifie directement la manière de travailler.

Le debugging ne se fait plus principalement en runtime, mais beaucoup plus tôt dans le cycle.

* En Spring, une application peut sembler stable jusqu’au premier usage réel
* Avec Quarkus, si l’application démarre, elle est généralement déjà cohérente

Les erreurs deviennent visibles plus tôt, dans un contexte maîtrisé.

=== Impact côté architecture

Sur le plan architectural, les effets sont concrets :

* comportement plus déterministe
* réduction des effets liés à l’environnement
* meilleure maîtrise des dépendances
* diminution des écarts entre local et production

L’approche build-time permet également de limiter :
...
* moins de tolérance
* plus d’exigence dès le développement

=== Modèle de programmation

Quarkus ne se limite pas au build-time.  
Il apporte aussi une flexibilité dans le modèle de programmation.

Une application peut combiner :
...
* simplicité pour les cas standards
* scalabilité pour les charges importantes
* compromis avec les threads virtuels

Cela permet une adoption progressive sans rupture technique.

=== Expérience développeur

L’expérience développeur est un autre point structurant.

Le mode de développement permet :

* modification du code sans redémarrage complet
* rechargement rapide
* feedback immédiat

Cela réduit fortement les cycles de développement et améliore la productivité.
...
|Feedback dev |Rapide |Progressif
|Risque production |Réduit |Plus élevé
|Comportement |Prévisible |Parfois implicite
|===

Le gain principal n’est pas uniquement la performance, mais la *prévisibilité du système*.

En pratique, cela se traduit par moins d’effets de bord et moins de surprises en production.
