...
Pour donner un peu de vie à notre application, profitons de ce qui est un véritable apport de JavaFX par rapport à Swing : les effets et les animations. +
...
Ces effets peuvent être appliqués aussi via FXML ou par API java directement. +
...
Et directement dans le code java :
...
Lorsque l'animation sera jouée, JavaFX se chargera de modifier progressivement la propriété `opacity` de notre image pour la faire passer de 0 à 1 en interpolant toutes les valeurs intermédiaires. +
